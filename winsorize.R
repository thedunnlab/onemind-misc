...
#' Adapted from https://github.com/matthieugomez/statar/blob/master/R/winsorize.R
#' 
...
#' @param IQR IQR range, set to 3 by default
#' @examples                          
#' v <- c(1:4, 99)
#' winsorize(v)
...
winsorize <- function(x, probs = NULL, cutpoints = NULL , replace = c(cutpoints[1], cutpoints[2]), verbose = TRUE, IQR = 3){
  dummy = is.integer(x)
  if (!is.null(probs)){
    ...
    cutpoints <- c(l[2]-IQR*(l[3]-l[1]), l[2]+IQR*(l[3]-l[1]))
  } else{
